...
class RealmCRUDViewModel {
    ...
    func updateBorrower(id:ObjectId,name:String){
        if isEmptyCheck(name) { return }
        
        try! realm.write {
            let borrowers = realm.objects(Borrower.self)
            if let borrower = borrowers.where({$0.id == id}).first {
                borrower.name = name
            }
        }
    }
    
    func updateFlagBorrower(id:ObjectId,flag:Bool){
        
        try! realm.write {
            let borrowers = realm.objects(Borrower.self)
            if let borrower = borrowers.where({$0.id == id}).first {
                borrower.returnFlag = flag
            }
        }
    }
    ...
    func readBorrowerList() -> Results<Borrower> {
        try! realm.write {
            let borrowers = realm.objects(Borrower.self)
            return borrowers.sorted(byKeyPath: "returnFlag",ascending: true)
        }
    }
